...
2. `rspdl-core`는 한국어 조사, 영어 어순 또는 Locale AST를 알지 않는다.
...
초기부터 phase마다 crate를 만들지 않는다. `rspdl-core` 내부 module boundary를 먼저 검증하고, 독립 배포나 빌드 격리가 필요해질 때 분리한다.
...
flowchart TD
    ...
    COMPILER --> CORE["rspdl-core"]
    KO --> CORE
    EN["future rspdl-en"] --> CORE
    COMPILER -. "future" .-> EN
```
...
초기 의미 백본은 [정규화 타입·도메인과 논리 IR 코어 RFC](rfcs/0002-typed-domains-and-logic-core.md)를 따른다. 모든 canonical value, variable, predicate와 set expression은 완전히 해석된 타입을 가지며 `Any`나 암시적 형변환을 허용하지 않는다. 유한 도메인과 계산 가능한 무한 도메인은 ground membership과 backend별 symbolic support를 명시해, Datalog/SMT lowering이 지원하지 않는 의미를 근사하지 않게 한다.
...
`rspdl-core`는 Locale source 없이 hand-authored IR fixture로 다음을 테스트한다.
...
- 최소 선언 문법과 stable machine ID 표기
- 0.1에 포함할 한국어 정책 문형
- 파일 확장자와 module header
- comment와 quoted identifier escape
- Canonical IR JSON schema
- diagnostic Rule ID naming convention
- parser recovery 수준과 error limit
