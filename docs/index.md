...
| `rspdl-language-prd` | `prd` | `draft` | [RSPDL Product Requirements](prd.md) | Defines the product and language requirements for turning explicit planning intent into deterministic, explainable implementation context. | `screen-structure-spec-divergence`, `data-lifecycle-modeling-gap`, `policy-consistency-blind-spots`, `semantic-source-provenance-loss` | `language-design`, `data-lifecycle`, `policy-analysis`, `semantic-ir`, `diagnostics`, `conformance` |
...
| `semantic-reference-direction-loss` | `problem` | `active` | [Semantic Reference Direction Loss](problems/0007-semantic-reference-direction-loss.md) | Canonical IR preserves resolved target IDs but not a navigable account of which semantic records refer to each target. | - | `semantic-ir`, `semantic-graph`, `downstream-navigation`, `impact-analysis` |
