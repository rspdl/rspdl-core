...
- 현재 구현 범위는 다음과 같다.
  ...
  - direct enum action 또는 immutable Event payload의 한국어 conditional ExactlyOne Create/Skip, enum coverage와 same-variant conflict, direct input·ExistingModel input field·constant의 무조건 또는 동일 decision enum variant `PreMutation` producer, 무조건 output-field-only message template와 variant별 required output field gap/conflict
