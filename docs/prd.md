...
- 언어와 호환성 요구사항은 다음과 같다.
  ...
  - `LOCALE-002`: Locale frontend는 source를 공통 Unlinked IR로 변환하고 symbol resolution, type checking과 의미 규칙은 공통 analyzer가 한 번만 구현해야 한다.
...
- 현재 구현 범위는 다음과 같다.
  ...
  - 공통 `Frontend` trait, Unlinked IR과 Locale 독립 symbol resolution, type checking 및 data usage analyzer
