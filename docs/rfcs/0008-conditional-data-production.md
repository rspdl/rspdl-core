...
### 구현된 ExactlyOne relation-slot producer

새 slot 선언 문법은 만들지 않는다. output model이 기존 binary relation의 첫 endpoint이고 해당 relation에 `Required`와 `Unique` cardinality constraint가 모두 성립할 때만 output-owned ExactlyOne slot으로 해석한다.

```rspdl
점검 전달 알림은 기술자를 수신자(recipient)로 가질 수 있다.
모든 점검 전달 알림은 수신자를 하나 이상 가져야 한다.
각 점검 전달 알림은 수신자를 최대 하나만 가질 수 있다.
수신자 연결(recipient_binding)은 점검 요청 전달이 실행될 때 수신 기술자를 점검 전달 알림의 수신자로 연결한다.
```

producer source는 같은 action의 direct `ExistingModel` input이며 second endpoint와 정확히 같아야 한다. conditional producer, fan-out, traversal/join, reversed/ternary relation, optional/many slot, runtime relation data와 snapshot은 지원하지 않는다.
